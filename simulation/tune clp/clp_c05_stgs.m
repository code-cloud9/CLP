...
r0 = 25; 
alpha_1 = 0.05:0.01:0.15;  % for BH procedure
alpha_2 = 0.1:0.02:0.3;  % for eBH procedure
m_deRand = 20;  % #replications for derandomize
sig = 1.5; % signal strength
...
for rep_idx = 1:reps
    ...
    [matrix_A_eg, matrix_M_eg, matrix_C_eg] = gen_data(graphon_setting, node_num, sig, missing_prob);  
    ...
    parfor m = 1:m_deRand
        ...
        for i0 = row_indices'
            ...
            for ii = 1:r2
                ...
                evalues_inaTestsplit = eValues(pvalues_inaTestsplit, (0.05+0.01*(idx_sig-1)));
    ...
    eBH_rej_set = eBH(0.5/alpha_1(idx_sig) * e_list_deRand, alpha_2(idx_sig), batch_size);
...
% Save results
save(['alpha_c0.5_stg' num2str(graphon_setting) '.mat'], 'res_example');
