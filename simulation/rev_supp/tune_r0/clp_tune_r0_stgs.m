...
sig = 0.3;                 % fixed signal strength
r0_list = [10, 20, 30, 40]; % varying r0 values
...
res_example = table( ...
    nan(reps, nr0), nan(reps, nr0), nan(reps, nr0), nan(reps, nr0), ...
    'VariableNames', {'TP', 'FP', 'FN', 'TN'} ...
);
...
for rep_idx = 1:reps
    ...
    for idx_r0 = 1:nr0
        ...
        [matrix_A_eg, matrix_M_eg, matrix_C_eg] = gen_data_unic( ...
            graphon_setting, node_num, sig, missing_prob);

        row_indices = find(any(matrix_M_eg == 1, 2));

        % ----- derandomize: get #m_deRand e_list(s) and take the average -----
        ...
        parfor m = 1:m_deRand
            ...
            for i0 = row_indices'
                ...
                [vector_train_idx, vector_calib_idx, vector_test_idx] = gen_partition( ...
                    node_num, i0, matrix_M_eg, train_proportion);
                ...
                for ii = 1:r2
                    ...
                    for j0 = one_split_test
                        ...
                        est_res_j0 = ests( ...
                            node_num, i0, matrix_A_eg, matrix_M_eg, ...
                            vector_train_idx, j0, calib_idx_set_j0);

                        pvalue_i0j0 = pvalue_single( ...
                            i0, j0, matrix_A_eg, matrix_C_eg, ...
        ...
        df_missing_idx = table( ...
            (1:size(missingLocation, 1))', missingLocation(:, 1), missingLocation(:, 2), ...
            'VariableNames', {'id', 'row', 'colmn'} ...
        );
        ...
        TP(idx_r0) = sum(sum(conditionTrue & rej_location));
        FP(idx_r0) = sum(sum(conditionFalse & rej_location));
        FN(idx_r0) = sum(sum(conditionTrue & ~rej_location));
        TN(idx_r0) = sum(sum(conditionFalse & ~rej_location));
...
% Save results
save(['clp_unic_stg' num2str(graphon_setting) '_vary_r0.mat'], 'res_example');
